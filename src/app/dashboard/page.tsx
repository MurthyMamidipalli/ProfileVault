...
export default function DashboardOverview() {
  ...
  const isProfileEmpty = !profile.fullName && !profile.bio && (profile.jobs || []).length === 0;
      ...
      <div className="flex flex-col items-center justify-center space-y-6 pt-12 text-center">
        ...
        <div className="space-y-2">
          <h1 className="text-4xl font-headline font-bold tracking-tight">
            Welcome, <span className="text-primary">{profile.fullName || "Vault Owner"}</span>
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto w-full">
        <Card className={cn(
          ...
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Cloud Sync Status</CardTitle>
          </CardHeader>
            ...
            {isSynced ? (
              <>
                <div className="flex items-center gap-2 text-accent font-bold">
                  ...
                  Real-Time Mirror Active
                </div>
                <p className="text-xs text-muted-foreground">
                  Your profile is automatically synced to the cloud. Changes made here appear instantly on your mobile and other browsers.
                </p>
                <div className="text-[10px] text-muted-foreground font-mono bg-white/5 p-2 rounded">
                  Last Update: {new Date(profile.lastSyncedAt!).toLocaleString()}
                </div>
              </>
            ) : (
              <>
                ...
                <p className="text-xs text-muted-foreground">
                  Establishing secure connection to your professional vault.
                </p>
              </>
            )}
          </CardContent>
        ...
        <Card className="glass-card border-l-4 border-l-primary">
          ...
          <CardContent className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              All personal data is encrypted and accessible only to your authenticated session. Your public portfolio only displays information you choose to share.
            </p>
          </CardContent>
        </Card>
      </div>
