...
export default function PublicProfileView() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20 selection:bg-primary selection:text-primary-foreground">
        ...
        <div className="max-w-6xl mx-auto px-6 h-full flex items-end pb-12 relative z-10">
          <div className="flex flex-col md:flex-row md:items-end gap-8 w-full animate-fade-in">
            <div className="relative shrink-0">
              ...
              <Badge className="absolute -bottom-2 -right-2 bg-primary text-primary-foreground px-4 py-1.5 text-[8px] font-black border-2 border-background shadow-2xl tracking-[0.2em] flex items-center gap-1.5">
      ...
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-16 mt-16">
        ...
        <div className="lg:col-span-4 space-y-12 animate-fade-in">
          <section className="space-y-6">
            <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-primary/60">Contact Network</h2>
          ...
          {jobs.length > 0 && (
            <section className="space-y-6">
              <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-primary/60">Active Roles</h2>
                  ...
                  <Card key={proj.id} className="glass-card border-none bg-white/[0.01] overflow-hidden group hover:bg-white/[0.03] transition-smooth">
                    ...
                    <CardHeader className="p-6 space-y-4">
                      <div className="space-y-2">
                        <h3 className="text-xl font-bold tracking-tight text-foreground">{proj.title}</h3>
                        {proj.date && (
                      ...
                      <div className="flex flex-wrap gap-3 pt-2">
                         {proj.url && (
                           <Button asChild size="sm" variant="secondary" className="h-8 bg-accent/10 text-accent hover:bg-accent hover:text-accent-foreground font-bold text-[10px]">
            ...
            <section className="space-y-12">
              ...
              <div className="space-y-16">
                {profile.experience.map((exp) => (
                  <div key={exp.id} className="relative pl-10 border-l-2 border-primary/10 group">
                    <div className="absolute top-0 left-[-7px] w-3 h-3 rounded-full bg-primary border-2 border-background shadow-[0_0_20px_rgba(var(--primary),0.8)] group-hover:scale-125 transition-smooth" />
                    <div className="space-y-6">
                      <div className="space-y-2">
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                          <h3 className="text-2xl md:text-3xl font-bold tracking-tight text-foreground">{exp.title}</h3>
                          <Badge className="w-fit bg-primary/10 text-primary border-none font-black tracking-[0.2em] text-[10px] px-4 py-1.5">
                            {exp.startDate.split('-')[0]} — {exp.endDate ? exp.endDate.split('-')[0] : 'PRESENT'}
                          </Badge>
                        </div>
                        <div className="flex items-center gap-4 text-xl font-semibold text-accent">
                          <span>{exp.company}</span>
                          {exp.location && (
                            <span className="text-muted-foreground text-[10px] font-black tracking-[0.2em] uppercase opacity-40">/ {exp.location}</span>
                          )}
                        </div>
                      </div>
                      
                      {exp.description && (
                        <p className="text-muted-foreground leading-relaxed text-base font-medium whitespace-pre-line max-w-4xl opacity-80">
                          {exp.description}
                        </p>
                      )}
                    </div>
            ...
            <section className="space-y-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {profile.education.map((edu) => (
                  <Card key={edu.id} className="glass-card border-none bg-white/[0.01] shadow-xl hover:bg-white/[0.03] transition-smooth group p-8">
                    <CardHeader className="p-0 pb-6 flex flex-row items-center gap-6">
                      <div className="p-4 bg-primary/10 rounded-2xl text-primary shrink-0 group-hover:scale-110 transition-smooth">
                        <GraduationCap className="w-8 h-8" />
                      </div>
                      <div className="space-y-1">
                        <CardTitle className="text-xl font-bold tracking-tight text-foreground leading-tight">{edu.degree}</CardTitle>
                        <p className="text-xs text-primary font-bold tracking-widest uppercase">{edu.institution}</p>
                      </div>
                    </CardHeader>
                    <CardContent className="p-0 space-y-6">
                      <div className="text-[9px] font-black tracking-[0.3em] text-muted-foreground/40 uppercase flex items-center gap-3">
                        <Calendar className="w-3.5 h-3.5" />
                        {edu.startDate.split('-')[0]} — {edu.endDate ? edu.endDate.split('-')[0] : 'PRESENT'}
                      </div>
                      {edu.description && (
                        <p className="text-muted-foreground italic leading-relaxed font-medium pl-6 border-l-2 border-primary/20 text-xs opacity-70">
                          {edu.description}
                        </p>
                      )}
                    </CardContent>
      ...
      <footer className="mt-32 py-24 border-t border-border/50 text-center space-y-8 bg-white/[0.01]">
        <div className="space-y-4">
          <p className="text-muted-foreground text-[10px] font-black uppercase tracking-[0.5em] opacity-30">Encrypted Profile Infrastructure</p>
          <div className="flex items-center justify-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center shadow-[0_10px_40px_rgba(var(--primary),0.4)]">
              <FolderCode className="w-6 h-6 text-primary-foreground" />
            </div>
            <span className="font-headline font-black text-3xl tracking-tighter text-foreground">
        ...
        <p className="text-[10px] text-muted-foreground/20 font-bold uppercase tracking-[0.3em]">
          &copy; {new Date().getFullYear()} ProfileVault Core Infrastructure
        </p>
      </footer>
    </div>
  );
